...
export default async function AdminPage() {
  try {
    ...
    const atRiskBatchesCount = await prisma.inventoryBatch.count({
      where: {
        expiryDate: { gte: now, lte: fiveDaysFromNow },
        quantity: { gt: 0 }
      }
    });
    ...
    let topRetailers = await Promise.all(topRetailersRaw.map(async (r) => {
      ...
      return {
        ...
        score: r._avg.compositeScore ? Math.round(r._avg.compositeScore * 100) : 0,
        orders: orderCount
      };
    }));
                      ...
                      {topRetailers.map((retailer, i) => (
                        <TableRow key={i} className="hover:bg-slate-50/80 transition-colors border-slate-100 group cursor-pointer">
                          <TableCell className="text-center py-4">
                            <div className={`mx-auto flex h-7 w-7 items-center justify-center rounded-full text-xs font-black shadow-sm ${i === 0 ? 'bg-gradient-to-br from-amber-100 to-amber-200 text-amber-700 ring-1 ring-amber-300' :
